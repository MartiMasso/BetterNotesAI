...
% =========================
% Preamble (from preamble.tex) — corrected for robustness
% =========================
...
\newcommand{\realn}{\mathbb{R}_n}
\newcommand{\cplexn}{\mathbb{C}_n}
...
% =========================
\linespread{0.95} % (use a single spacing knob; removed baselinestretch duplication)
...
\begin{obs}
    If $p(x) \in \realn[x]$, we write $p(x) = \sum_{k=0}^n a_{k} x^{n-k}$.
\end{obs}
...
\begin{proposition}[Newton's rule]
    If $p(x) \in \realn[x]$ and $L \in \real$ satisfies that $p(L), \ p'(L), \ldots, p^{(n)}(L)$ are positive (or negative),
    then $L$ is an upper bound for the real roots of $p(x)$.
\end{proposition}

\begin{proposition}
    Let $p(x) \in \cplexn[x]$ and $z \in \cplex$ be a root of $p(x)$. Then
...
\begin{obs}
    ...
    \begin{itemize}[leftmargin=*]
        ...
        $\overline{L} > 0$ upper bound for real roots of $p(x) \Rightarrow -\frac{1}{\overline{L}}$ upper bound for negative real roots of $p(x)$.
    \end{itemize}
\end{obs}
...
$\mathcal{F}_n = \{ f_n = a_0 \varphi_0(x) + \cdots + a_n \varphi_n(x) \mid a_0, \ldots,a_n \in \real \}$.
Given $f$ on $I$, we want $f_n^*\in \mathcal{F}_n$ s.t.
$\norm{f - f_n^*}_2 = \min_{f_n\in\mathcal{F}_n} \norm{f-f_n}_2$.

\begin{theorem}
    Given $I$ and l.i.\ $\{\varphi_i(x)\}_{i=0,\ldots,n}$, $\exists! f_n^* = \sum_{k=0}^n a_k^* \varphi_k(x)$ minimizing $\norm{f-f_n}_2$ in $\mathcal{F}_n$.
\end{theorem}
...
From $\{\varphi_j\}_{j=0,\ldots,n}$ we obtain orthogonal $\{ \psi_j \}_{j=0,\ldots,n}$ (Gram-Schmidt). Setting up the system
\vspace{-5pt}
\[
    (\varphi_0(x),\ldots,\varphi_n(x)) = (\psi_0(x),\ldots,\psi_n(x))
    \begin{pmatrix}
    ...
    \vspace{-5pt}
\]
with $r_{ij} = \frac{\langle \varphi_j, \psi_i \rangle}{d_i}$, where $d_i = \langle \psi_i, \psi_i \rangle$.
...
\begin{obs}
    ...
    $f_n^* = c_0^* \psi_0(x) + \cdots + c_n^* \psi_n(x)$.
\end{obs}

\begin{proposition}
    If $\psi_0(x),\ldots,\psi_n(x)$ are orthogonal, $f_n^*(x) = \sum_{j=0}^n c_j^* \psi_j(x)$ is the least-squares function, and
    $\norm{f-f_n^*}^2 = \norm{f}^2 - \norm{f_n^*}^2$.
\end{proposition}
...
\begin{itemize}
    ...
    \item Given $\psi_0,\ldots,\psi_i$ with degrees $\deg(\psi_j)=j$, orthogonal $\{\psi_j\}_j$ ($\Rightarrow$ l.i.),
    any polynomial $p_i$ of degree $i$ can be written uniquely as
    $p_i(x) = c_0 \psi_0(x) + \cdots + c_i \psi_i(x)$.
